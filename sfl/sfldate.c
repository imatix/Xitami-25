...
timestamp_now (void)
{
    return (make_timestamp (date_now (), time_now ()));
}

/*  ---------------------------------------------------------------------[<]-
    Function: gmtimestamp_now

    Synopsis: Get current GMT date and time and encode into a real value (8 bytes).
    ---------------------------------------------------------------------[>]-*/
...
gmtimestamp_now (void)
{
    ...
    return (make_timestamp (date, time));
}
...
make_timestamp (long date, long time)
{
    ...
    sprintf (buffer, "%ld.%07ld", date_to_days  (date),
                                  time_to_csecs (time));
...
timestamp_date (double timestamp)
{
    ...
    sprintf (buffer, "%08.07f", timestamp);
    buffer [7] = '\0';
    date = atol (buffer);
