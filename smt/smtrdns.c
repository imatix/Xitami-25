...
#include "smtrdnsl.h"                   /*  Reverse DNS library              */
...
MODULE get_host_message_value (THREAD *thread)
{
    ...
    exdr_read (thread-> event-> body, SMT_GET_HOST_NAME, &tcb-> ip_address,
               &tcb-> ip_value, &tcb-> user_tag);
    ...
    while (user_data && (void *)user_data != (void *)&tcb-> reply)
      {
        ...
        SEND (&user_data-> reply_to, "HOST_ERROR", "");
        user_data = next_data;
      }
}
...
MODULE get_answer_result (THREAD *thread)
{
    ...
    host = rdns_read_answer (tcb-> query, (short)tcb-> readsize,
                             tcb-> cur_request, tcb-> invalid_ns_tab,
                             tcb-> rr_result, tcb-> rr_result_nbr);
...
MODULE signal_end_of_search (THREAD *thread)
{
    ...
    while (user_data && (void *)user_data != (void *)&tcb-> reply)
      {
        next_data = user_data-> next;
        SEND (&user_data-> reply_to, "HOST_END", "");
        user_data = next_data;
      }
}
...
MODULE get_ip_message_value (THREAD *thread)
{
    ...
    exdr_read (thread-> event-> body, SMT_GET_HOST_IP, &tcb-> host_name,
                                      &tcb-> user_tag);
    tcb-> reply_to = thread-> event-> sender;
    tcb-> main_req_type = REQ_TYPE_IP;
}
...
MODULE send_ip_address_value (THREAD *thread)
{
    ...
    while (user_data && (void *)user_data != (void *)&tcb-> reply)
      {
        next_data = user_data-> next;
        msg_size = exdr_writed (&msg, SMT_HOST_IP,
...
MODULE signal_timeout (THREAD *thread)
{
    ...
    while (user_data && (void *)user_data != (void *)&tcb-> reply)
      {
        next_data = user_data-> next;
        SEND (&user_data-> reply_to, "HOST_TIMEOUT", "");
        user_data = next_data;
      }
}
