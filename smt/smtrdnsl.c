...
#include "smtrdnsl.h"                   /*  Function prototypes              */
...
QUERY_BUF *
rdns_make_query (char *host_ip, long id, int *size, Bool recursive)
{
    QUERY_BUF
        *query = NULL;
    byte
    ...
    *size = (int)(data - query-> query_buffer);
    return (query);
}
...
QUERY_BUF *
rdns_make_ip_query (char *host_name, long id, int *size, Bool recursive)
{
    QUERY_BUF
        *query = NULL;
    byte
...
Bool
rdns_read_answer (QUERY_BUF *answer, short size, NS_REQUEST *current,
                     ...
                     if (c & 0200)
                       {
                          servp = NULL;
                          if (protop)
                              servp = getservbyport(htons((unsigned short) n),
                                                    protop-> p_name);
