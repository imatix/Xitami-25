/*---------------------------------------------------------------------------*
 *  fxtest2.i - LIBERO dialog interpreter for fxtest2.c.                     *
    ...
    if (_LR_load_context () == 0)       /*  If no context, initialise        */
      {
        ...
        session-> form = form_init (&form_fxtest2, TRUE);
        if (session-> form == NULL)
            return (FEEDBACK_ERROR);
        form_data = (FXTEST2_DATA *) session-> form-> data;
        ...
        if (the_next_event == _LR_show_event)
          {
            ...
            if (form_changes == -1)
              {
                fxput_text (FXTEST2_MESSAGE_TO_USER, form_strerror);
                _LR_show_the_form (TRUE);
                return (FEEDBACK_SHOW);
              }
            else
                fxput_text (FXTEST2_MESSAGE_TO_USER, "");
...
_LR_enter_state (void)
{
    ...
    if (the_next_event == _LR_null_event)
      {
        coprintf ("fxtest2: No event set after event %d in state %d",
                             ctx._LR_event, ctx._LR_state);
        return (1);                     /*  End dialog program               */
      }
    else
    if (ctx._LR_event >= 6 || ctx._LR_event < 0)
      {
        coprintf ("fxtest2: State %d - event %d is out of range",
    ...
    if (ctx._LR_index == 0)
      {
        coprintf ("fxtest2: State %d - event %d is not accepted",
                         ctx._LR_state, ctx._LR_event);
        return (1);                     /*  End dialog program               */
      }
    the_next_event          = _LR_null_event;
...
_LR_load_context (void)
{
    ...
    form = form_init (&form_fxtest2, FALSE);
    data = form-> data;
    ...
    form-> defn = &form_fxtest2;        /*    reload the form item           */
    session-> form = form;
    return (1);
}
