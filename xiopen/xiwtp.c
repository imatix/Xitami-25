/*===========================================================================*
 *  Name:       xiwtp.c                                                      *
 *  Title:      Xitami/WTP Transaction Server                                *
 *                                                                           *
...
#define SERVER_NAME "Xitami/WTP/" VERSION

#define USAGE                                                               \
    "Syntax: xiwtp [options...]\n"                                          \
    "Options:\n"                                                            \
    ...
    "                   Xitami/WTP on port 5080, do 'xiwtp -b 5000'\n"      \
    ...
    "  -v               Show Xitami/WTP version information\n"              \
...
int wtpman_init (void);                 /*  WTP service agent                */
...
main (int argc, char *argv [])
{
    ...
    if (argparm)
      {
        puts ("Argument missing - type 'xiwtp -h' for help");
        exit (EXIT_FAILURE);
      }
    else
    if (!args_ok)
      {
        puts ("Invalid arguments - type 'xiwtp -h' for help");
        exit (EXIT_FAILURE);
      }
    ...
    smt_init ();
    server_name = "Xitami/WTP";

    /*  Load the agents we want to use                                       */
    wtpman_init ();                     /*  WTP service agent                */
